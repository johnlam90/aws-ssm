...
func (s *stubLoader) LoadInstances(ctx context.Context, q *SearchQuery) ([]Instance, error) { return s.instances, nil }
func (s *stubLoader) LoadInstance(ctx context.Context, id string) (*Instance, error) { return s.single, nil }
...
func TestCachedInstanceLoader(t *testing.T) {
    ...
    if err != nil { t.Fatalf("cache service err: %v", err) }
    ...
    if err != nil || len(res) != 1 { t.Fatalf("first load failed: %v %v", err, res) }
    // Second load should hit cache (cannot assert directly; rely on speed)
    res2, _ := cl.LoadInstances(ctx, &SearchQuery{})
    if len(res2) != 1 { t.Fatalf("second load mismatch") }
    // Single instance load
    inst, err := cl.LoadInstance(ctx, "i-123")
    if err != nil || inst.InstanceID != "i-123" { t.Fatalf("single load failed") }
