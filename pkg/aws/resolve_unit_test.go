...
func TestUnitResolveSingleSuccess(t *testing.T) {
    ...
    if err != nil || got.InstanceID != "i-123" { t.Fatalf("expected instance, got=%v err=%v", got, err) }
}
...
func TestUnitResolveSingleMultiple(t *testing.T) {
    ...
    if err == nil { t.Fatalf("expected multiple instances error") }
}
...
func TestUnitResolveSingleNotRunning(t *testing.T) {
    ...
    if err == nil { t.Fatalf("expected not running error") }
}
