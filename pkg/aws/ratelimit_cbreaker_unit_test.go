...
func TestUnitCircuitBreakerTransitions(t *testing.T) {
    ...
    if cb.GetState() != CircuitOpen { t.Fatalf("expected open") }
    clk.Advance(11 * time.Millisecond)
    if err := cb.Allow(); err != nil { t.Fatalf("allow error: %v", err) }
    if cb.GetState() != CircuitHalfOpen { t.Fatalf("expected half-open") }
    cb.RecordSuccess(); cb.RecordSuccess()
    if cb.GetState() != CircuitClosed { t.Fatalf("expected closed") }
}
...
func TestUnitCircuitBreakerReopen(t *testing.T) {
    ...
    if cb.GetState() != CircuitHalfOpen { t.Fatalf("expected half-open") }
    cb.RecordFailure()
    if cb.GetState() != CircuitOpen { t.Fatalf("expected reopen") }
}
